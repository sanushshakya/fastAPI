...
def get_api_router(app):
    ...
    @router.get("/product/{_id}", response_description="Get Product")
    ...
    @router.put("/product/{_id}", response_description="Update Product")
    async def update_product(id: str, request: Request, product: UpdateProductModel = Body(...)):
        ...
        if len(product) >= 1:
                ...
                if update_result.modified_count == 1:
                    if (
                        updated_product := await request.app.mongodb["products"].find_one(
                        {"_id": id}
                        ) 
                    )is not None:
    ...
    @router.delete("/product/{_id}", response_description="Delete Product")
